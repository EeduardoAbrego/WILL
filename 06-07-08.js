...
function crearClasePersona() {
  class Persona {
    ...
    getFriends() {
      ...
      // Tu código aca:
      var listaAmigos = []
      this.amigos.map((obj) => {listaAmigos.push(obj.nombre)})
      return listaAmigos
    ...
    getPromedioEdad() {
      ...
      // Tu código aca:
     var  edadAmigos = []
      this.amigos.map((obj) => {edadAmigos.push(obj.edad)})
      var suma = 0
      for (let i = 0; i < edadAmigos.length; i++) {
        suma += edadAmigos[i] 
      }
      return suma / edadAmigos.length
